# CERN-CMS-ECAL
Code to analyse the prototype of CMS ECAL for the upcoming high luminosity LHC upgrade. Studies the amplitude, the amplitude difference and the time difference between signals in the channels of the detector. Built as a Python package, 'ecalautoanalysis', with three main classes to use: Amplitude, Amplitude_Delta and Time_Delta, each being subclasses of ECAL.
...
When changing the fitting method (eg. from 'synchronise' to 'gaussian'), one must make sure that the previous "Statistics" folder is either empty or inexistent. Otherwise, the code will just load the data and not re-generate them to save computation time. 
